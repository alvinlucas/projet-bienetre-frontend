...
describe("Home", () => {
    ...
    test("affiche le slogan", () => {
        expect(
            screen.getByText(/Prenez une pause, inspirez et vivez pleinement l'instant présent/i)
        ).toBeInTheDocument();
    });
    ...
    test("affiche la section 'Nos Points Forts'", () => {
        expect(screen.getByRole("heading", { name: /Nos Points Forts/i })).toBeInTheDocument();
    ...
    test("affiche la section 'Rejoignez Notre Communauté'", () => {
        expect(screen.getByRole("heading", { name: /Rejoignez Notre Communauté/i })).toBeInTheDocument();
    ...
    test("affiche le footer avec l'année actuelle", () => {
        ...
        expect(
            screen.getByText(new RegExp(`© ${currentYear} Respirer Son Bien-Être`, "i"))
        ).toBeInTheDocument();
    });
});
