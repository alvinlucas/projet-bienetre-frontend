...
const Videos = () => {
    ...
    const handleAddVideo = async () => {
        if (!newVideo.titre || !newVideo.description || !newVideo.url || !newVideo.datePublication) {
        ...
        try {
            ...
            setError(err.response?.data?.message || "Erreur lors de l'ajout de la vidéo.");
        }
    };
        ...
        <div className="bg-beige min-h-screen">
            ...
            <div className="p-8">
                <div className="flex justify-between items-center mb-6">
                    <h1 className="text-3xl font-playfair text-green-900 mx-auto">Mes Vidéos</h1>
                    {user?.isAdmin && (
                        <button
                            ...
                            <div className="p-6">
                                ...
                                <p className="text-sm text-green-500">
                                    {(() => {
                                        ...
                                        const diffTime = Math.ceil((expirationDate - now) / (1000 * 60 * 60 * 24));

                                            return (
                                                <>
                                                    Expire dans : <span
                                                    className="font-semibold">{diffTime} jour{diffTime > 1 ? "s" : ""}</span>
                                                </>
                                            );
                                    })()}
                                </p>
            ...
            {showModal && (
                <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center">
                    <div className="bg-white p-8 rounded-lg shadow-lg w-96">
                        <h2 className="text-2xl font-semibold mb-4 text-green-900">Ajouter une Vidéo</h2>
                        {error && <p className="text-red-500 mb-4">{error}</p>}
                        <form>
                            <div className="mb-4">
                                <label className="block text-lg font-semibold text-green-900">Titre</label>
                                <input
                                    type="text"
                            ...
                            <div className="mb-4">
                                <label className="block text-lg font-semibold text-green-900">Description</label>
                                <textarea
                            ...
                            <div className="mb-4">
                                <label className="block text-lg font-semibold text-green-900">URL</label>
                                <input
                                    type="text"
                                    value={newVideo.url}
                            ...
                            <div className="mb-4">
                                <label className="block text-lg font-semibold text-green-900">Date de Publication</label>
                                <input
                                    type="date"
                                    value={newVideo.datePublication}
                                    onChange={(e) =>
                                        setNewVideo({ ...newVideo, datePublication: e.target.value })
                                    }
                                    className="w-full border border-green-300 rounded px-3 py-2 focus:outline-none focus:ring focus:ring-green-500"
                                />
                            </div>
